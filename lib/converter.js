...
async function getImageBlob(url) {
  ...
  const request = (async () => {
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch slide image (${res.status})`);
    }

    const contentType = res.headers.get("content-type") || "";

    if (!contentType.startsWith("image/")) {
      throw new Error(`Invalid slide response: ${contentType}`);
    }
...
async function generatePDF(slides, onProgress) {
  ...
  // Return as a Blob instead of immediately downloading it, so callers
  // (background prep or direct export) decide what to do with it.
...
export async function preparePDF(slides, onProgress) {
  // Images may already be prefetched by page.tsx, but getImageBlob()
  // has its own cache/dedup, so this is safe either way — and now
  // forwards progress so a click that joins this in-flight promise
  // can still show live numbers instead of a frozen progress bar.
  return generatePDF(slides, onProgress);
}

/* ---------------------------------------------------------
   PDF DOWNLOAD (fresh generation, no background prep available)
--------------------------------------------------------- */

export async function exportToPDF(slides, filename, onProgress) {
  const blob = await generatePDF(slides, onProgress);
